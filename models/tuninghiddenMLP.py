...
profits = []
for i in range(len(params['activation'])):
            
            clf = MLP(random_state=7, solver='lbfgs', hidden_layer_sizes=(75,), activation='relu', alpha=1e-7)
            ...
            profits.append((profit1, params['activation'][i], np.mean(losses)))
            
X = [a[1] for a in profits]
loss_plot = [a[2] for a in profits]
profit_plot = [a[0] for a in profits]

'''plt.subplot(2,2,1)
plt.plot(X, loss_plot)
plt.subplot(2,2,2)
plt.plot(X, profit_plot)
plt.show()'''
