...
params = {'C': [1e-1, 1, 5, 10], 'gamma': [1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 5e-2, 1e-1, 5e-1]}

profits = []
for i in range(len(params['C'])):
    for j in range(len(params['gamma'])):
        clf = SVC(random_state=7, C=params['C'][i], gamma=params['gamma'][j])
        ...
        profits.append((profit1, params['C'][i], params['gamma'][j]))

profits_s = sorted(profits, key=lambda x: x[0], reverse=True)
print("  1st place:", {'profit': profits_s[0][0], 'C': profits_s[0][1], 'gamma': profits_s[0][2]})
print("  2nd place:", {'profit': profits_s[1][0], 'C': profits_s[1][1], 'gamma': profits_s[1][2]})
print("  3rd place:", {'profit': profits_s[2][0], 'C': profits_s[2][1], 'gamma': profits_s[2][2]})
print("  4th place:", {'profit': profits_s[3][0], 'C': profits_s[3][1], 'gamma': profits_s[3][2]})
print("  5th place:", {'profit': profits_s[4][0], 'C': profits_s[4][1], 'gamma': profits_s[4][2]})

# 1st place: (-208, 1, 0.5)
# 2nd place: (-224, 10, 0.1) -- stable top-5
# 3rd place: (-308, 10, 0.01)
# 4th place: (-474, 10, 0.05)
# 5th place: (-492, 1, 0.05)
